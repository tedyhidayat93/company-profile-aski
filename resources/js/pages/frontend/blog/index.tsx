        ...
        <FrontendLayout>
            ...
            {isSearchOpen && (
                <div className="fixed inset-0 bg-slate-950/70 backdrop-blur-sm z-50 flex items-start justify-center pt-24 px-4 transition-all animate-in fade-in duration-200">
                    <div className="bg-white w-full max-w-xl rounded-2xl p-6 shadow-2xl border border-slate-200 animate-in slide-in-from-top-4 duration-200">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Cari Artikel</h3>
                            <button onClick={() => setIsSearchOpen(false)} className="p-1 rounded-lg text-slate-400 hover:bg-slate-100 transition cursor-pointer">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <form onSubmit={handleSearch} className="flex gap-2">
                            <div className="relative flex-1">
                                ...
                                <Input
                                    ...
                                    className="h-12 pl-11 text-base rounded-xl bg-slate-50 border-slate-200 focus-visible:ring-orange-500 font-bold"
                                />
                            </div>
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 bg-white text-slate-900 font-sans antialiased">
                
                {/* 📌 HEADER UTAMA + KAPSUL NAVIGASI KATEGORI */}
                <div className="flex flex-col gap-6 mb-10 pb-6 border-b border-slate-150">
                    ...
                    <div className="flex items-center max-w-full overflow-x-auto no-scrollbar py-1.5 relative">
                        
                        {/* 📌 Bagian yang Di-fixed/Sticky (Tombol Cari + Pembatas) */}
                        <div className="sticky left-0 z-10 flex items-center gap-2.5 bg-white pr-2">
                            {/* Tombol Cari */}
                            <button 
                            ...
                            <span className="h-6 w-px bg-slate-200 mx-1 shrink-0" />
                        </div>
                ...
                {isFiltered && (
                    <div className="mb-8 p-4 bg-slate-50 rounded-xl border border-slate-200/60 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 animate-in fade-in duration-300">
                        <div className="text-sm font-medium text-slate-600">
                            Menampilkan hasil untuk:{' '}
                            {filters.search && <span className="font-extrabold text-slate-900">Kata Kunci "{filters.search}"</span>}
                            {filters.category && <span className="font-extrabold text-slate-900">Kategori "{activeCategoryName || filters.category}"</span>}
                            {filters.tag && <span className="font-extrabold text-slate-900">Tag #{filters.tag}</span>}
                            <span className="ml-2 px-2.5 py-0.5 rounded-md bg-slate-200 text-xs text-slate-600 font-bold">
                ...
                <section aria-label="Artikel Pilihan Utama" className="grid grid-cols-1 md:grid-cols-12 gap-5 mb-14">
                    {/* 🟧 KARTU UTAMA (KIRI) */}
                    {headline_posts[0] && (
                        <article className="md:col-span-8 group relative aspect-[16/10] md:aspect-auto md:h-[420px] w-full rounded-2xl overflow-hidden bg-slate-950 shadow-md border border-slate-100">
                            <img
                    ...
                    <div className="md:col-span-4 grid grid-cols-1 gap-5">
                        {headline_posts.slice(1, 3).map((post) => (
                            <article key={post.id} className="group relative aspect-[16/9.5] md:aspect-auto md:h-[200px] w-full rounded-2xl overflow-hidden bg-slate-950 shadow-md border border-slate-100">
                                <img
                ...
                {/* 🌟 SECTION: ARTIKEL POPULER & TERBARU */}
                    ...
                    <section aria-label="Artikel Terpopuler dan Terbaru" className="mb-16 animate-in fade-in duration-500">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-2 mb-6 border-b py-5">
                            <div>
                                ...
                                <h2 className="text-xl font-black text-slate-950 tracking-tight">Informasi Container Terpopuler Untuk Anda</h2>
                            </div>
                            <p className="text-sm text-slate-500 max-w-sm md:text-right font-medium leading-relaxed">
                                            ...
                                            <div className="flex items-center justify-between gap-4 pt-1">
                                                ...
                                                {most_read_posts[0].category && (
                                                    <button 
                                                        ...
                                                        className="inline-flex items-center rounded-full border border-slate-200 bg-white px-3.5 py-1 text-xs font-bold text-slate-600 shadow-3xs hover:border-orange-500 hover:text-orange-500 cursor-pointer transition-all duration-200"
                                                    >
                                        ...
                                        <div className="flex-1 flex flex-col justify-between h-24 sm:h-28 py-0.5">
                                            ...
                                            {post.category && (
                                                <button 
                                                    ...
                                                    className="self-start inline-flex items-center rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-bold text-slate-600 shadow-3xs hover:border-orange-500 hover:text-orange-500 cursor-pointer transition"
                                                >
                                                    {post.category.name}
                                                </button>
                                            )}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
                    ...
                    <div className="lg:col-span-8 space-y-8">
                        <div className="border-b border-slate-200 pb-3 mb-4">
                            <h2 className="text-sm font-black tracking-wider text-slate-950 uppercase flex items-center gap-2">
                                    ...
                                    <article key={post.id} className="group flex flex-col space-y-3.5">
                                        ...
                                        <div className="flex-1 flex flex-col justify-between space-y-2">
                                            ...
                                            <div className="pt-2 flex items-center justify-between text-xs text-slate-400 font-medium border-t border-slate-100">
                                                <span className="truncate max-w-[100px] text-slate-600 font-semibold">
                    ...
                    <aside className="lg:col-span-4 space-y-8 w-full border-t lg:border-t-0 lg:border-l border-slate-200 pt-8 lg:pt-0 lg:pl-8">
                        ...
                        {random_products.length > 0 && (
                            <div className="space-y-4 pt-2">
                                <div className="flex items-center gap-2 text-slate-950 border-b border-slate-100 pb-3">
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    {random_products.map((product) => (
                                        <Link 
                                            ...
                                            className="group flex flex-col bg-slate-50 rounded-xl overflow-hidden border border-slate-200/50 p-2 hover:bg-slate-100/60 transition-all shadow-xs"
                                        >
                                            <div className="aspect-[4/3] w-full rounded-lg overflow-hidden relative bg-slate-200 mb-2">
                                                <img 
                                                    src={product.image} 
                                                    alt={product.name}
                                                    onError={handleImageError}
                                            ...
                                            <div className="flex-1 flex flex-col justify-between">
                                                <h4 className="text-xs font-extrabold text-slate-900 line-clamp-2 leading-tight group-hover:text-orange-500 transition-colors">
                                                    {product.name}
                                                </h4>
                                                {product.category && (
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    {popular_tags.map((tag, index) => (
                                        <button 
                                            ...
                                            className={`rounded-lg px-2.5 py-1 text-xs font-bold transition-all cursor-pointer ${filters.tag === tag ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                                        >
                                            #{tag}
                                        </button>
                                    ))}
