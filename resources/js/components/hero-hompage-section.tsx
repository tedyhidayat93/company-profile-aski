...
export default function HeroSection({
  ...
  return (
    <section id="home" className="w-full bg-white dark:bg-slate-950 pt-4 pb-12 px-4 sm:px-6 lg:px-8">
      {/* 🔲 CONTAINER UTAMA: Menggunakan Slate Pekat (Mendekati Hitam/Abu Baja) agar Aksen Oranye Menyala */}
      <div className="max-w-[94vw] mx-auto relative bg-slate-900 dark:bg-slate-950 border border-slate-800 rounded-[2.5rem] lg:rounded-[3.5rem] p-4 sm:p-6 lg:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 min-h-[600px] lg:min-h-[680px] overflow-hidden items-stretch">
        ...
        <div className="col-span-1 lg:col-span-6 relative w-full h-[400px] lg:h-auto rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden group shadow-2xl border border-slate-800">
          ...
          {activeProduct && (
            <div className="absolute bottom-6 left-6 right-6 z-20 bg-slate-900/90 dark:bg-slate-950/95 backdrop-blur-md p-4 rounded-2xl flex items-center justify-between gap-4 border border-slate-800 shadow-xl transition-all duration-300">
              <div className="flex flex-col min-w-0">
                <span className="text-[10px] font-black tracking-widest text-orange-500 uppercase mb-0.5">Featured Unit</span>
                <h3 className={`font-black text-white text-sm sm:text-base line-clamp-1 transition-opacity ${isFade ? 'opacity-40' : 'opacity-100'}`}>
                  {activeProduct.name}
                </h3>
              </div>
              ...
              <Link 
                ...
                className="shrink-0 inline-flex h-9 items-center justify-center rounded-xl bg-orange-500 hover:bg-orange-600 text-white px-4 text-xs font-extrabold transition-all gap-1.5 group/btn"
              >
